...
const EvolutionOfCyber = () => {
  ...
  const [devService, setDevService] = useState(false); 
  const [prodService, setProdService] = useState("");
  
  // Handler for Development Service change
  const handleDevServiceChange = (e) => {
    setDevService(e.target.value);
    if (e.target.value === "None") {
      setProdService(""); // Reset product service if 'None' is selected
    }
  };

  // Handler for Product Service change
  const handleProdServiceChange = (e) => {
    setProdService(e.target.value);
    if (e.target.value === "None") {
      setDevService(""); // Reset development service if 'None' is selected
    }
  };
